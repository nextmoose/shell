                ...
                lib =
                  (
                    nixpkgs : scripts : hook :
					  ...
					  _utils.visit
					    {
					      set = track : builtins.trace "02 : ${ builtins.concatStringsSep " , " ( builtins.map ( value : if value then "Y" else "N" ) ( builtins.attrValues track.reduced ) ) } - ${ builtins.typeOf track.reduced.program3 }" ( builtins.all ( x : x ) track.reduced ) ;
